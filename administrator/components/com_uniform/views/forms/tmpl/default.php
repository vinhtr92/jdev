...
<?php
$modal_params = array();
$modal_params['title'] = JTEXT::_("JSN_UNIFORM_IMPORT");
$modal_params['closeButton'] = true;
$modal_params['animation'] = true;
$body = '
    <p class="text-left" style="padding: 10px">'.JTEXT::_("JSN_UNIFORM_IMPORT_NOTE").' <a href="'.JRoute::_('index.php?option=com_uniform&view=configuration&s=maintenance&g=data').'">>>></a></p>
    <form name="" method="POST" class="form-inline text-center" enctype="multipart/form-data" action="'.JRoute::_('index.php?option=com_uniform&view=forms&task=forms.import').'">
    <input type="file" name="file" accept=".json"  class="form-control" id="jsn-uniform-import-file" required>
    <input type="checkbox" name="keep_user_id"> '.JTEXT::_("JSN_UNIFORM_IMPORT_KEEP_USER_ID").'
    <button type="submit" class="btn btn-primary">'.JTEXT::_("JSN_TOOLBAR_IMPORT").'</button> 
    <br/>
    <p class="text-left" style="padding: 10px">'.JTEXT::_("JSN_UNIFORM_IMPORT_KEEP_USER_ID_DESC").'</p>
   </form>';

echo JHTML::_('bootstrap.renderModal', 'jsn-uniform-import-modal', $modal_params, $body);
