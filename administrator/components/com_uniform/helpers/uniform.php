	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false, $isModule = false)
	{
		...
		if ($items)
		{
			...
			if (!$formType)
			{
				...
				$html .= $topContent;
				$html .= "<form name='form_{$formName}' data-ajaxupload='0' id='form_{$formName}' action=\"" . 'JSN_UNIFORM_ROOT_URL' . '/index.php?option=com_uniform&amp;view=form&amp;task=form.save&amp;form_id=' . $items->form_id . "\" method=\"post\" class=\"form-validate {$classForm} \" enctype=\"multipart/form-data\" autocomplete=\"off\">";
				...
				if ($i + 1 == count($formPages))
				{
					...
					if (!empty($items->form_captcha) && $items->form_captcha == 1 && (! (int) $items->form_hide_captcha_if_logged_in || ! (int) $user->get('id')))
					{
						...
						if(version_compare(@$params->version, '2.0', '>=')){
                            ...
                            $html .= '<script type="text/javascript">try {grecaptcha.render("' . $captchaId . '", {sitekey: "' . $dataSiteKey . '", theme: "' . $dataTheme . '", timeout:'.$reCaptchaTimeout.'});'
                                . '}catch(err){}</script>';
			...
			if (!$formType)
			{
				...
				$html .= "<input type=\"hidden\" id=\"ajax_upload_file\" name=\"ajax_upload_file\" value=\"0\" />";
