...
class PlgSystemPageBuilder3 extends JPlugin
{
    ...
    public static $unSupport = array('com_quick2cart');
    ...
    public function onAfterInitialise()
    {
        ...
        if (!in_array($option, self::$unSupport)) {
            defined('PAGEFLY_VERSION') || define('PAGEFLY_VERSION', '1.3.3');
...
//    }
    ...
    public function onAjaxPageBuilder3()
    {
        require_once dirname(__FILE__) . '/includes/ajax.php';
    ...
    {
        // Instantiate PB3 Load Module plugin.
        if (class_exists('PlgContentPB3LoadModule') || JPluginHelper::importPlugin('content', 'pb3loadmodule', false)) {
            ...
            $plugin = new PlgContentPB3LoadModule($dispatcher, (array)$plugin);

            // Emulate an article.
            $article = ( object )array('text' => $temp);
            $params = array();

            // Trigger onContentPrepare event to let the PB3 Load Module plugin render all {pb3loadmodule} tags.
