...
class plgSystemJsnpoweradmin extends JPlugin
{
	...
	function __construct(&$subject, $config)
	{
		...
		{
			$unSupportedComponentList 	= json_decode(JSNPA_UNSUPPORTED_COM_LIST, true);
			$option					 	= $input->getVar('option', '');
			if (in_array($option, $unSupportedComponentList))
			{
				$this->_params->set('enable_adminbar', false);
			}
		}
	...
	public function onBeforeRender()
	{
		...
		if(!$this->_application->isAdmin() && $this->_params->get('custom_assets_enhance', true)){
			...
			if(count((array) $menuCss) && isset($menuCss->assets)){
			...
			if(count((array) $menuJs) && isset($menuJs->assets)){
	...
	public function onAfterRender(){
		...
		if ($document instanceOf JDocumentHTML) {
			...
			if ($this->_application->isAdmin() && $this->_user->id > 0) {
				...
				if ($option == 'com_poweradmin' && ($view == 'changeposition' || $view == 'rawmode' || $view == 'search' || $view == 'configuration' || $view == 'about' || $view == 'templates' || $view == 'positionlisting' || $view == 'installer'  || $view == 'selectmoduletypes' || $view == 'module' || $view == 'selectmenutypes' || $view == 'menuassets'))
				{
					...
					if ($view == 'module')
					{
						$content = preg_replace('#<script[^>]+src="' . $uri . '/media/system/js/html5fallback.js(\?(.*))?"[^>]*></script>#', '', $content);
						$content = preg_replace('#<script[^>]+src="' . $uri . '/media/system/js/moduleorder.js(\?(.*))?"[^>]*></script>#', '', $content);
					}
				}
	...
	public function onAfterRoute(){
			...
			if(($option == 'com_modules' && $view == 'module') || ($option == 'com_advancedmodules' && $view == 'module') || ($layout == 'edit' && $option == 'com_poweradmin' && $view == 'module')){
				...
				if($this->_params->get('position_chooser_enhance', true)){
					if((!$input->getInt('id', 0) || ($module && $module->client_id == 0)) && $this->canEnablePosChooser()){
						...
						JSNHtmlAsset::addScript($uri . '/plugins/system/jsnpoweradmin/assets/js/window.js?v=' . PoweradminHelper::getVersion());
						JSNHtmlAsset::addScript($uri . '/plugins/system/jsnpoweradmin/assets/js/module/position.js?v=' . PoweradminHelper::getVersion());
						$this->_document->addStyleSheet($uri . '/plugins/system/jsnpoweradmin/assets/css/window.css?v=' . PoweradminHelper::getVersion());
						$customScript = "
		...
		if ($this->_application->isAdmin() && $this->_user->id > 0)
		{
			...
			{
				JSNHtmlAsset::addScript(JUri::root(true).'/plugins/system/jsnpoweradmin/assets/js/jsn.noconflict.3rd.js?v=' . PoweradminHelper::getVersion());
			}
		}
