	...
	protected function getListQuery()
	{
		...
		if (JPluginHelper::isEnabled('content', 'vote'))
		{
			...
			$query->select(
				'COALESCE(NULLIF(ROUND(v.rating_sum  / v.rating_count, 0), 0), 0) AS rating,
					COALESCE(NULLIF(v.rating_count, 0), 0) as rating_count')->join(
				'LEFT', '#__content_rating AS v ON a.id = v.content_id');
		}
		...
		if (count($categoryId))
		{
			...
			foreach ($categoryId as $filter_catid)
			{
				...
				$subCatItemsWhere[] = '(' . ( $level ? 'c.level <= ' . ( (int) $level + (int) $categoryTable->level - 1 ) . ' AND ' : '' ) .
